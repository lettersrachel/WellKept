...
const MARKS = new Map(Object.entries({
  ...
  "G-13": ["blocks-E1", "THE GATE NOBODY HAS LOOKED AT SINCE AUGUST. The staff disclosure is founder-approved and neither counsel-reviewed nor acknowledged, and CLAUDE.md's merge gate says no capture surface attributing data to a named HOM ships before it. RECLASSIFIED 5 September by ruling: a HIRING PRECONDITION on the 25 September agenda, not a document to finish. Owed against surfaces that SHIPPED, since `time_entry` and `object_observation` already exist under that rule; unenforceable in CI; and reached by the founding cohort rather than by a queue row, which is the combination that let it sit."],
  ...
  "G-53": ["blocks-E1", "The reveal audit records the ATTEMPT and never the outcome, so a failed reveal and a successful one are indistinguishable in the trail counsel's packet calls load-bearing. UNBLOCKED 5 September: the outcome vocabulary is ruled as four closed values, and the build is queue row Q-11l."],
