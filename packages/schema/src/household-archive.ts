...
export const CORPORATE_ONLY: Record<string, string> = {
  ...
  changeset: "Q-12b-2 reconciliation, the same reason as expected_event: the row is the company's own propagation work rather than a fact about the household, and it carries a CLASSIFICATION of whether a change was safe to act on without asking, which is machinery a member archive has no business carrying.",
  changeset_effect: "Q-12b-2, and it is engine bookkeeping one level below the changeset: which internal work rows one change invalidated, and why. Nothing here is a fact the household stated or would recognise.",
  fallback_plan: "Q-12b-3, and it stays corporate for the reason its sibling does plus one of its own. The row is the company's own readiness work, and it carries a computed statement of what the household's Decision Rights PERMIT without asking, which is authority machinery: handing a member a row that says a step was permitted invites reading it as a decision that was made, when nothing was executed at all. The household's own grant is already theirs through decision_right; this is our reading of it.",
