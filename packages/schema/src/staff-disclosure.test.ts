...
const ALLOWLIST: Record<string, string> = {
  ...
  fallback_plan:
    "Q-12b-3, 2026-09-05: recorded_by is write provenance and is the ONLY staff column on this table. The evaluation that decides which step a plan reached writes no person at all (the time_segment posture), deliberately: which rung a household's own grant reaches is a fact about the grant, never about the operator who looked. So what is attributed is the recording of a plan and never the reading of one. Joins the founder line candidates for the next revision under the same internal-observations item as its two siblings.",
  changeset:
