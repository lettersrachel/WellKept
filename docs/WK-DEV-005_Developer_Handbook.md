...
- Deleting: nothing hard-deletes BY DEFAULT. Households archive; fields tombstone; audit is append-only. Membership-end deletion is a scheduled job with an export + certificate, not a DELETE. CORRECTED 5 August 2026 (the commissioning brief's housekeeping item 2; same correction CLAUDE.md received in PR #104): documented, reasoned exceptions DELETE rows, each named with its reason in erase-household.mjs's own header (vault_item, condition_flag, object_observation, paused_decision, notification, field_event_outbox, and audit_subject_token, added by AR's G-59 fix, where deleting the mapping IS the audit-identity erasure mechanism per ADR-006). A new exception needs a written reason in the erasure tool, never a silent addition.
