...
| Incident / complaint register | **gap — recommendation pending** | No record anywhere of a client complaint, breakage, injury, or near-miss. Recommended 2026-07-25 (LAUNCH.md §3): a dedicated append-only `incident_report` table (not a registry kind), surfaced on the drill-in + fleet board. Awaiting founder go-ahead to build |
...
| Photo lifecycle | **gap — recommendation pending** | Photos are live: base64 in Postgres (`visit_photo`), staff-only + MFA on read, never in the client view; `photoRefs` (REQ-012) unused, media-reuse flags (REQ-006/071) not built. Recommended 2026-07-25 (LAUNCH.md §3): 90-day rolling purge of image bytes with tombstoned metadata and an incident-hold exemption; disclose in the privacy notice. Needs counsel + the founder's number before build |
