...
| 011 25 section slots (0-24), N/A-confirmed first-class | P0 | **partial.** CORRECTED 1 August 2026 (WK-DEV-001): the standard is 25 slots numbered 0-24, not 24 fixed sections. Section 0 is record-level (field-level decline as a confirmed fact, plus the Record Preview). Sections 1-24 are the Playbook's content sections; WK-SOP-000 Addendum A3's "24-section Playbook" name still refers to those and is unaffected by the correction. SECTION_NAMES (packages/schema/src/enums.ts:34) has no slot 0, only 1-24, and its own comment still reads "The 24 fixed sections." Slot 0 is unbuilt; nothing enforces non-renumbering beyond convention either |
...
| 076 deletion on request, non-client records | P0 | **not built.** ADDED 1 August 2026 (WK-DEV-001), governed by WK-STD-026. Covers records held about recipients, vendors, neighbours and Member Circle entries, people who never consented and cannot see their own record; REQ-071's deletion clause reaches a client household at membership end only and does not cover these. Scope: hard delete on request, hard delete after 24 months of inactivity by a scheduled job, the deletion itself audited, and a non-marketable constraint excluding these rows from every export and analytics view by default. Checked packages/schema/src: no member_circle table, non-client model, or 24-month job exists. BLOCKER: no record about a person who is not a client may be created before this exists, which gates WK-SVC-007 Showing Up and the lead pipeline (a lead who never signs is also a non-client) |
