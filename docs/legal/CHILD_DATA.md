...
| `decision_right` (Q-6-1, 2026-09-04, same-PR) | a Decision Right can name a child directly: the seed's own non-negotiable list reads "anything touching a child's school medical or activity enrollment", and a household may write its own rule about a child's activities or pickup | internal (s2); NO client projection exists at all, since the member-facing surface is FREEZE-GATED (WK-DEV-007, Part C s2.2); free-text policy applies as everywhere | code (no client render path) + payload guard (decision_right signature, any recognizable row a violation) |
...
| `commitment_ledger_item` (Q-6-2, 2026-09-05, same-PR) | a commitment or a member decision may concern a child (a school form to return, an activity enrollment, a pediatric appointment to confirm) | internal (s2); never shown to any client, the member decision inbox being freeze-gated; free-text policy applies as everywhere | code (no client render path) + payload guard (commitment_ledger_item signature) |
...
| `mail_outcome` (Q-1, 2026-09-04, migration 0062) | only indirectly: a delivery event's payload echoes the subject of mail WE composed, and our client copy names the household, never a child | provider delivery evidence, corporate-only; never rendered to any client; deleted whole on household erasure (the tenth DELETE exception) | code (no client render path) + erasure DELETE |
| `expected_event` (Q-12b-1, 2026-09-05, migration 0069) | only if an operator types one in: the expectation is free text, and a school-cycle packet expectation could name a child's school. No column holds a child's data by design, and the six patterns are about vendors, registrations, refunds and packets rather than about people | corporate-only reconciliation, SHADOW: it is visible on the household drill-in and on no member surface and in no HOM briefing; free text blanked to a marker on household erasure, structure kept (the work_item class) | code (no client render path; the client-payload-shape guard has no expected_event key) + erasure blanking |
| `changeset` (Q-12b-2, 2026-09-05, migration 0070) | only if an operator types one in: the change is described in free text, and the spec's own example (a child's soccer moving from Saturday to Sunday) is exactly a case where a child's activity would be named | corporate-only reconciliation, SHADOW: visible on the household drill-in, on no member surface and in no HOM briefing; free text blanked to a marker on household erasure, structure kept | code (no client render path) + erasure blanking |
| `changeset_effect` (Q-12b-2, 2026-09-05, migration 0070) | no free text about a person: each row names a dependent work requirement and why the change reaches it | corporate-only, SHADOW; DELETED whole on household erasure (the eleventh DELETE exception), before its parent, since its composite foreign key points at `changeset` | code (no client render path) + erasure DELETE |
| `fallback_plan` (Q-12b-3, 2026-09-05, migration 0073) | only if an operator types one in: the choice a plan is for and its four options are free text, and a fallback for who collects a child from an activity would name both the child and the person | corporate-only reconciliation, SHADOW: visible on the household drill-in, on no member surface and in no HOM briefing; nothing is executed from a plan, so no fallback ever reaches a person as an instruction; free text blanked on household erasure and the structure kept, with the choice marker carrying the row id so the one-plan-per-choice unique index survives the blanking | code (no client render path; the client-payload-shape guard has no fallback_plan key) + erasure blanking |
