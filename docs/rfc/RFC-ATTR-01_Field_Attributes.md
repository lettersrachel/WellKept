...
| `latest_safe_start` (computed date) | **Absent; new migration required per the ruling**, with an open shape question recorded rather than decided: the repository's standing pattern for derived values of this kind is computed-never-stored (flag promotion, time_segment duration), which would make this migration-free. Whether it stores or computes is the queue item's first question for the founder; the ruling's wording is not reworded here. |
