...
through `N-1`, so 23 applied = `0000`…`0022`. State counts as "N in the
...
before a deploy carries dead client JS and stale server-action IDs, and its
...
**Checks 1, 4, and 12 are scripted**, so run them first, then work the
manual ones:
...
3. On the FIXTURE drill-in: approve the seeded pending client edit on the
   ...
   corporate_admin, and the `field_write` row is the proof, not the panel
4. Dev-gated surfaces are actually gated, and BOTH return 404 in production:
...
6. A household drill-in shows the **Household consent card**: a red
...
10. Both briefing surfaces show the "Last year at this time" section (its
    empty-state note counts, since recall is dark until a year of history)
...
12. `app_setting` rows exist in production with intended values:
    ...
    (`{"actRateFloor":0.25,"minHouseholds":3,"minUsers":2}`), so insert
    ...
    the script asserts its presence and shape but never repairs it, because the
...
- **A page that ends abruptly** (cards you know exist just aren't there,
  ...
  at its time ceiling MID-STREAM, so the browser keeps whatever HTML
...
- **After every deploy, refresh open tabs** (and tell any active user to)
  because a tab loaded before the deploy carries dead client JS and stale
...
- **A deploy order is only executable against a named main sha.** Three
  ...
  BEFORE `db:migrate`, because the migrate-then-deploy coupling is exactly where
...
- **A docs-only merge still moves the build id**, and the G-37 skew banner
