 ...
 *  - decision_record (RFC-PRIM-01, 2026-08-25): free text BLANKED
 *    (question, recommendation, alternatives, evidence, outcome note),
 *    skeleton kept - THAT a choice was routed and how it ended is
 *    operational history; what the choice concerned is the household's.
 ...
 *    whole-or-absent CHECK survives), skeleton kept - the
 *    household_task_profile posture: how they wanted it done is the
 *    household's words; THAT preferences were kept and pruned is
 *    operational history.
 ...
 *    what the work was about is the household's. Blanking preserves the
 *    lifecycle CHECKs (a resolved row keeps a marker resolution, never
 *    NULL).
 ...
 *  - estimate_snapshot (WL Gate 1, 2026-08-25): basis BLANKED, rows
 *    kept - internal planning history (the time_entry-adjacent class:
 ...
 *    household's, while the tombstoned skeleton keeps referential
 *    shape. No business-record claim survives in the words.
 *  - visit_brief_snapshot (WK-DEV-009 s2.1, 2026-08-25): payload
 *    BLANKED, skeleton kept - THAT a brief was composed, when, for
 *    whom, under which projection is operational evidence (the
 *    reconstructability the snapshot exists for); the payload is the
 *    household's content verbatim. The content hash stays, the
 *    audit-row value-hash precedent.
...
console.log(`  estimate snapshots to BLANK (basis; rows kept):             ${counts.estimateSnapshots}`);
...
try {
  ...
  // decision_record: blank the words, keep the lifecycle (see header).
  ...
  // visit_brief_snapshot (WK-DEV-009 s2.1, 2026-08-25): payload BLANKED,
  // skeleton and content hash kept (see header).
  await c.query(`UPDATE visit_brief_snapshot SET payload='{"erased": true}'::jsonb, updated_at=now() WHERE household_id=$1`, [householdId]);
